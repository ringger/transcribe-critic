...
def generate_markdown(config: SpeechConfig, data: SpeechData) -> None:
    ...
    # Inputs: merged or ASR transcript, slide timestamps, slides JSON
    ...
    # Determine layout mode based on available data
    has_slide_timestamps = (
    ...
    has_segment_timestamps = (
        data.transcript_segments and
        len(data.transcript_segments) > 0
    )

    if has_slide_timestamps:
        print("  Using timestamp-based slide placement")
        content = _generate_interleaved_markdown(data)
    elif has_segment_timestamps:
        print(f"  Using timestamped transcript layout ({len(data.transcript_segments)} segments)")
        content = _generate_timestamped_markdown(data)
    else:
...
def _generate_interleaved_markdown(data: SpeechData) -> str:
    ...
    Note: Interleaved mode uses ASR segments for timestamps.
    ...
    # Note about source
    source_note = "ASR transcript"
    if data.merged_transcript_path and data.merged_transcript_path.exists():
        source_note = "ASR transcript (merged version available in transcript_merged.txt)"
...
def _generate_sequential_markdown(data: SpeechData) -> str:
    ...
    # Add footer
    source_note = "merged transcript" if data.merged_transcript_path else "ASR transcript"
    lines.extend([
        "---",
        "",
        f"*Generated by transcriber.py (source: {source_note})*",
...
def _generate_timestamped_markdown(data: SpeechData) -> str:
    """Generate markdown with periodic timestamps from ASR segments.

    Groups segments into ~60-second paragraphs, each prefixed with a timestamp.
    Uses the merged transcript text when available for better quality, with
    timestamps derived from ASR segment boundaries.
    """
    lines = [
        f"# {data.title}",
        "",
    ...
    # Use the best available transcript text for content
    transcript_text = _get_best_transcript_text(data)
    if not transcript_text:
        return _generate_sequential_markdown(data)

    # Build timestamp markers from segments at ~60s intervals
    markers = []  # list of (char_offset_approx, timestamp_str)
    last_ts = -60.0
    for seg in data.transcript_segments:
        t = seg.get("start", 0.0)
        if t - last_ts >= 60.0:
            markers.append((t, fmt_duration(t)))
            last_ts = t

    if not markers:
        return _generate_sequential_markdown(data)

    # Split the transcript text into paragraphs at ~60s intervals.
    # Strategy: use segment word counts to map ASR timing onto the merged text.
    seg_words = []
    for seg in data.transcript_segments:
        seg_words.append(len(seg.get("text", "").split()))

    # Map each marker to a cumulative word offset in ASR segments
    marker_word_offsets = []
    cum_words = 0
    marker_idx = 0
    for i, seg in enumerate(data.transcript_segments):
        t = seg.get("start", 0.0)
        while marker_idx < len(markers) and markers[marker_idx][0] <= t:
            marker_word_offsets.append(cum_words)
            marker_idx += 1
        cum_words += len(seg.get("text", "").split())
    # Remaining markers
    while marker_idx < len(markers):
        marker_word_offsets.append(cum_words)
        marker_idx += 1

    # Split merged text by word offsets
    words = transcript_text.split()
    for i, (_, ts_str) in enumerate(markers):
        start_word = marker_word_offsets[i]
        end_word = marker_word_offsets[i + 1] if i + 1 < len(marker_word_offsets) else len(words)
        chunk = " ".join(words[start_word:end_word]).strip()
        if not chunk:
            continue
        lines.append(f"**[{ts_str}]**")
        lines.append("")
        lines.append(_format_paragraph(chunk))
        lines.append("")
